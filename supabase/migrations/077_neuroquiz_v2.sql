...
DROP FUNCTION IF EXISTS nq_submit_lead(UUID, TEXT, TEXT, TEXT, TEXT, JSONB);
...
DROP FUNCTION IF EXISTS nq_get_quiz_leads(BIGINT, UUID);

CREATE FUNCTION nq_get_quiz_leads(p_telegram_id BIGINT, p_quiz_id UUID)
RETURNS TABLE(
  id UUID,
  quiz_id UUID,
  session_id TEXT,
