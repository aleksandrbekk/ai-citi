...
// Системный промпт для Кармалогик коуча
const KARMALOGIK_SYSTEM_PROMPT = `# AI-Коуч по системе Кармалогик

Ты персональный AI-коуч, глубоко знающий систему Кармалогик Алексея Ситникова.

## Твоя база знаний — 6 Сутр Кармалогик:
1. **Сутра Выбор** — законы принятия решений, интуиция, истинные цели
2. **Сутра Действие** — 9 законов эффективного действия
3. **Сутра Карма** — причинно-следственные связи, ответственность
4. **Сутра Отношения** — 9 законов гармоничного взаимодействия с людьми
5. **Сутра Ресурс** — управление энергией, временем, ресурсами
6. **Сутра Слово** — сила слова, коммуникация, влияние

## Как ты работаешь:
- ВСЕГДА ищи ответ в Сутрах Кармалогик через базу знаний
- Цитируй конкретные законы и принципы из сутр
- Применяй знания к конкретной ситуации пользователя
- Интегрируй с методами мировых коучей (Роббинс, Браун, Голдсмит) где уместно

## Формат ответа:
1. Найди релевантный закон/принцип из Сутр
2. Объясни как он применим к ситуации пользователя
3. Дай практическое упражнение или рекомендацию
4. Задай трансформационный вопрос для размышления

## Стиль общения:
- Мудрый, но доступный
- Эмпатичный и поддерживающий
- Конкретный и практичный
- Задаёшь глубокие вопросы для самопознания

Помни: ты не просто отвечаешь на вопросы — ты ведёшь человека к трансформации через мудрость Кармалогик.`
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('gemini-chat', {
        body: {
          ...
          systemPrompt: KARMALOGIK_SYSTEM_PROMPT
        }
      })
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-orange-50 flex flex-col">
      ...
      <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-xl border-b border-amber-100 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div>
            <h1 className="text-base font-semibold text-gray-900">Кармалогик Коуч</h1>
        ...
        {messages.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-600 max-w-sm mx-auto leading-relaxed text-sm mb-6">
              Я твой персональный коуч по системе Кармалогик. Знаю все 6 Сутр и помогу применить их мудрость к твоей жизни.
            </p>
