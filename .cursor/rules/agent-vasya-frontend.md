...
**🚨 КРИТИЧЕСКИ ВАЖНО: FULLSCREEN РЕЖИМ TELEGRAM**

В fullscreen режиме Telegram оставляет фиксированные кнопки вверху экрана:
- **Кнопка "Закрыть" (X)** — слева
- **Кнопка "3 точки" (меню)** — справа

**ПРАВИЛО:** ВСЕГДА размещай контент ПОД этими кнопками, чтобы они не перекрывались!

**Как правильно:**

1. Используй функцию `getContentSafeAreaTop()` из `@/lib/telegram`:
```tsx
...
2. Или используй CSS переменную (если доступна):
```tsx
<div style={{ paddingTop: 'var(--tg-content-safe-area-inset-top, 56px)' }}>
  {/* Контент */}
</div>
```

3. Fallback для старых версий SDK:
```tsx
...
**НЕ ДЕЛАЙ:**
- ❌ `top: 0` без учета кнопок
- ❌ Использование только `env(safe-area-inset-top)` — этого недостаточно!
- ❌ Фиксированные значения типа `pt-4` — они не учитывают кнопки Telegram
